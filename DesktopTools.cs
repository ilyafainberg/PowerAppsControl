...
// -----------------------------------------------------------------------------
//  PowerAppsControl — MCP tool implementations
//
...
//
//  This file defines the tools exposed by the PowerAppsControl MCP server:
//
...
public static class DesktopTools
{
    ...
    [Description(
        ...
        "  • FFmpeg must be available (on PATH, or via the POWERAPPSCONTROL_FFMPEG env var). Install: " +
    ...
    public static string RecordWindow(
        ...
        int fps = 15,
        [Description("Optional output .mp4 path. Default: %USERPROFILE%\\Videos\\PowerAppsControl\\<title>_<timestamp>.mp4.")]
        string? outputPath = null)
    {
        ...
        {
            var dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyVideos), "PowerAppsControl");
    ...
    /// foreground window — PowerAppsControl runs un-elevated by default).
    /// </summary>
    ...
    // =========================================================================
    //  Testing-layer engine surface (internal)
    //
    //  The UX-testing tools (TestingTools / TestRunner) need to drive a SPECIFIC
    //  window BY HANDLE — a query string is ambiguous when several copies of the
    //  same app are open for a load run. These internal helpers expose the same
    //  low-level primitives the public tools use (SendInput click / scroll /
    //  keys, DWM frame math, PrintWindow capture, UI-Automation lookup + invoke)
    //  but target an explicit HWND and return structured results instead of the
    //  agent-facing strings. They deliberately skip the Enforcement gate: the
    //  TestSessionManager guarantees every window it drives is under agent
    //  control (crimson frame) for the duration of the session.
    // =========================================================================

    /// <summary>A resolved top-level window (handle + identity) for the testing layer.</summary>
    internal readonly record struct AppWindow(IntPtr Hwnd, string Title, uint Pid, string ProcessName);

    /// <summary>Resolve a free-text query to a single window (throws with candidates on miss).</summary>
    internal static AppWindow ResolveOne(string query)
    {
        var w = ResolveWindowOrThrow(query);
        return new AppWindow(w.Hwnd, w.Title, w.Pid, w.ProcessName);
    }

    /// <summary>
    /// Resolve up to <paramref name="max"/> DISTINCT visible windows matching a query, for
    /// multi-window load runs. Exact title/process matches come first, then substring matches.
    /// Throws (with a candidate list) when nothing matches at all.
    /// </summary>
    internal static List<AppWindow> ResolveMany(string query, int max)
    {
        if (max < 1) max = 1;
        var windows = EnumerateTopLevelWindows();
        var procQuery = query.EndsWith(".exe", StringComparison.OrdinalIgnoreCase) ? query[..^4] : query;
        ...
        var ordered = windows
            .Select(w => new
            {
                W = w,
                Rank =
                    string.Equals(w.Title, query, StringComparison.OrdinalIgnoreCase) ? 0 :
                    string.Equals(w.ProcessName, procQuery, StringComparison.OrdinalIgnoreCase) ? 1 :
                    w.Title.Contains(query, StringComparison.OrdinalIgnoreCase) ? 2 :
                    w.ProcessName.Contains(procQuery, StringComparison.OrdinalIgnoreCase) ? 3 : -1,
            })
            .Where(x => x.Rank >= 0)
            .OrderBy(x => x.Rank)
            .Select(x => new AppWindow(x.W.Hwnd, x.W.Title, x.W.Pid, x.W.ProcessName))
            .Take(max)
            .ToList();

        if (ordered.Count == 0) ResolveWindowOrThrow(query); // throws with candidates
        return ordered;
    }

    /// <summary>Bring a window forward (restoring it first if minimized) so input reaches it.</summary>
    ...
    /// <summary>The window's true DWM extended frame bounds (falls back to GetWindowRect).</summary>
    internal static Rectangle FrameOf(IntPtr hwnd)
    {
        ...
        return new Rectangle(r.Left, r.Top, Math.Max(1, r.Width), Math.Max(1, r.Height));
    }

    /// <summary>Capture a window to a bitmap (PrintWindow, screen-copy fallback). Caller disposes.</summary>
    internal static Bitmap CaptureWindow(IntPtr hwnd)
    {
        var f = FrameOf(hwnd);
        var bmp = new Bitmap(f.Width, f.Height, PixelFormat.Format32bppArgb);
        bool ok;
        using (var g = Graphics.FromImage(bmp))
        {
            var hdc = g.GetHdc();
        ...
        {
            using var g = Graphics.FromImage(bmp);
            g.CopyFromScreen(new Point(f.Left, f.Top), Point.Empty, new Size(f.Width, f.Height), CopyPixelOperation.SourceCopy);
        }
        return bmp;
    }

    /// <summary>Click at WINDOW-relative pixels on a specific handle (focuses it first).</summary>
    internal static void ClickWindow(IntPtr hwnd, int imageX, int imageY, string button = "left", int count = 1)
    {
        if (count < 1) count = 1;
        var f = FrameOf(hwnd);
        if (imageX < 0 || imageX >= f.Width || imageY < 0 || imageY >= f.Height)
            throw new ArgumentOutOfRangeException(nameof(imageX), $"({imageX},{imageY}) is outside the window frame ({f.Width}×{f.Height}).");

        Focus(hwnd);
        int sx = f.Left + imageX, sy = f.Top + imageY;
        if (!Win32.SetCursorPos(sx, sy))
            throw new InvalidOperationException($"SetCursorPos({sx},{sy}) failed (Win32 error {Marshal.GetLastWin32Error()}).");
        ...
        var inputs = new List<Win32.INPUT>(count * 2);
        for (int i = 0; i < count; i++) { inputs.Add(MouseEvent(down)); inputs.Add(MouseEvent(up)); }
        SendInputs(inputs);
    }

    /// <summary>Scroll the wheel over a specific window (optionally at a window-relative point).</summary>
    internal static void ScrollWindow(IntPtr hwnd, int amount, bool horizontal, int? imageX, int? imageY)
    {
        Focus(hwnd);
        if (imageX.HasValue && imageY.HasValue)
        {
            var f = FrameOf(hwnd);
            Win32.SetCursorPos(f.Left + imageX.Value, f.Top + imageY.Value);
        }
        ...
        SendInputs(new[]
        {
            ...
            {
                type = Win32.INPUT_MOUSE,
                U = new Win32.INPUTUNION { mi = new Win32.MOUSEINPUT { mouseData = unchecked((uint)delta), dwFlags = (uint)flag } }
            }
        });
    }

    /// <summary>Focus a window and type a key sequence into it (same syntax as send_keys).</summary>
    internal static int TypeInto(IntPtr hwnd, string keys)
    {
        Focus(hwnd);
        Thread.Sleep(40); // let focus settle before the first keystroke
        var inputs = KeySequenceParser.Parse(keys);
        SendInputs(inputs);
        return inputs.Count;
    }

    /// <summary>A UI-Automation element match with its center in physical pixels.</summary>
    internal readonly record struct UiaMatch(string ControlType, string Name, string AutomationId,
        int CenterX, int CenterY, bool Enabled, bool Offscreen);

    /// <summary>
    /// Find matching UIA elements under a window (exact match, then a case-insensitive
    /// substring fallback on Name). Returns structured results the runner can assert on.
    /// </summary>
    internal static List<UiaMatch> FindElements(IntPtr hwnd, string? name, string? automationId, string? controlType, int cap = 25)
    {
        var raw = FindRaw(hwnd, name, automationId, controlType, cap);
        var outList = new List<UiaMatch>(raw.Count);
        foreach (var el in raw)
        {
            try
            {
                ...
                outList.Add(new UiaMatch(
                    c.ControlType?.LocalizedControlType ?? "?",
                    c.Name ?? "", c.AutomationId ?? "",
                    (int)Math.Round(r.X + r.Width / 2.0), (int)Math.Round(r.Y + r.Height / 2.0),
                    c.IsEnabled, c.IsOffscreen));
            }
            catch { /* element vanished mid-read; skip */ }
        }
        return outList;
    }

    /// <summary>Invoke the first matching element via the best UIA pattern (or a real click fallback).</summary>
    internal static (bool ok, string info) InvokeElementByQuery(IntPtr hwnd, string? name, string? automationId, string? controlType)
    {
        var raw = FindRaw(hwnd, name, automationId, controlType, 1);
        if (raw.Count == 0) return (false, "no matching element");
        var el = raw[0];
        try
        {
            var r = el.Current.BoundingRectangle;
            int cx = (int)Math.Round(r.X + r.Width / 2.0);
            int cy = (int)Math.Round(r.Y + r.Height / 2.0);
            var action = InvokeElement(el, cx, cy);
            return (true, $"{action} at ({cx},{cy})");
        }
        catch (Exception ex) { return (false, ex.Message); }
    }

    /// <summary>The foreground window's handle (whatever the user is currently focused on).</summary>
    internal static IntPtr ForegroundWindow() => Win32.GetForegroundWindow();

    /// <summary>Identity of a specific handle (title + process), for the testing layer.</summary>
    internal static AppWindow Describe(IntPtr hwnd)
    {
        int len = Win32.GetWindowTextLength(hwnd);
        var sb = new StringBuilder(len + 1);
        Win32.GetWindowText(hwnd, sb, sb.Capacity);
        Win32.GetWindowThreadProcessId(hwnd, out var pid);
        return new AppWindow(hwnd, sb.ToString(), pid, GetProcessName(pid));
    }

    /// <summary>
    /// Read the URL from a browser window's address bar via UI Automation (the omnibox is an
    /// Edit control named "Address and search bar" in Edge/Chrome). Returns null when the window
    /// is not a browser or the value cannot be read.
    /// </summary>
    internal static string? ReadBrowserUrl(IntPtr hwnd)
    {
        try
        {
            ...
            if (bar is null)
            {
                ...
                foreach (AutomationElement e in edits)
                {
                    try
                    {
                        if ((e.Current.Name ?? "").Contains("Address", StringComparison.OrdinalIgnoreCase)) { bar = e; break; }
                    }
                    catch { /* skip */ }
                }
            }
            ...
            if (bar.TryGetCurrentPattern(ValuePattern.Pattern, out var p) && p is ValuePattern vp)
            {
                var v = vp.Current.Value;
                return string.IsNullOrWhiteSpace(v) ? null : v;
            }
        }
        catch { /* UIA can throw on protected surfaces */ }
        return null;
    }

    /// <summary>Known Power Platform hosts / substrings that identify a Power App URL.</summary>
    private static readonly string[] PowerAppHosts =
    {
        "apps.powerapps.com", "make.powerapps.com", "create.powerapps.com", "web.powerapps.com",
        "powerapps.com/play", "apps.gov.powerapps.us", "apps.high.powerapps.us",
        ".powerappsportals.com", ".powerpages.microsoft.com", ".microsoftcrmportals.com",
        ".dynamics.com",
    };

    /// <summary>
    /// Heuristic: does this URL (and optionally window title) look like a Power App — a canvas /
    /// model-driven app, the maker portal, or a Power Pages site? Returns the matched marker.
    /// </summary>
    internal static (bool isPowerApp, string reason) LooksLikePowerApp(string? url, string? title)
    {
        var u = (url ?? "").ToLowerInvariant();
        foreach (var host in PowerAppHosts)
            if (u.Contains(host)) return (true, $"URL host matches '{host.TrimStart('.')}'");

        // A model-driven app URL is <org>.crm*.dynamics.com/main.aspx?...appid=...
        if (u.Contains(".dynamics.com") && (u.Contains("appid=") || u.Contains("/main.aspx")))
            return (true, "URL looks like a model-driven app (Dataverse)");

        var t = (title ?? "").ToLowerInvariant();
        if (t.Contains("power apps") || t.Contains("powerapps"))
            return (true, "window title mentions Power Apps");

        return (false, string.IsNullOrWhiteSpace(u)
            ? "could not read a URL from the browser address bar"
            : $"URL '{url}' is not a recognized Power Platform host");
    }

    /// <summary>Core UIA walk shared by FindElements / InvokeElementByQuery. Exact then substring-on-name.</summary>
    private static List<AutomationElement> FindRaw(IntPtr hwnd, string? name, string? automationId, string? controlType, int cap)
    {
        ...
        try
        {
            var found = root.FindAll(TreeScope.Descendants | TreeScope.Element, cond);
            foreach (AutomationElement el in found)
            {
                result.Add(el);
        ...
        catch { /* tree changed mid-walk */ }

        // Substring fallback on Name (PropertyCondition is exact-match only).
